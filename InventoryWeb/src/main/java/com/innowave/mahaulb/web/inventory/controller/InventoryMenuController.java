...
private final String prefixURL = "inventory/masters";
    ...
    public String viewMaterialMapping(Locale locale,ModelMap model,HttpServletRequest req){
	...
	public ModelAndView searchStore(Locale locale, ModelMap model, HttpServletRequest req) {
		int ulbId = getSessionUser().getUlbId();
		MaterialStoreForm materialform = (MaterialStoreForm) req.getSession().getAttribute("materialstoreform");
		MaterialStoreForm materialStoreForm= new MaterialStoreForm();
		...
		List<TmInvStore> invStores= masterMaterialStoreServ.getAllMaterialStores(ulbId);
		List<TmCmDepartment> cmDepartments=masterMaterialStoreServ.getAllDepartments(ulbId);
		materialStoreForm.setCmDepartments(cmDepartments);
	...
	@RequestMapping(value = "/searchmaterialtype", method = RequestMethod.GET)
	 public ModelAndView searchMaterialTyp(Locale locale,ModelMap model){
		...
		Set<TmInvMaterialType> tmInvMaterialTypes = new HashSet<TmInvMaterialType>(0);
		...
		model.addAttribute("ulbId",getSessionUser().getUlbId());
		return new ModelAndView(prefixURL+"/master-search-materialtype","command",model);
    }
	...
	{
		List<TmInvMaterialType> detailsLst = new ArrayList<TmInvMaterialType>();
		detailsLst=masterMaterialTypServ.getMaterialTypeList(getSessionUser().getUlbId(),"Y");
		return detailsLst;
	}
}
